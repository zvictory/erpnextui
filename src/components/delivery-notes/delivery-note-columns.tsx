...
import { MoreHorizontal, Pencil, Trash2, Send, X, FileText } from "lucide-react";
...
export function getDeliveryNoteColumns(
  actions: ColumnActions,
  perms: ColumnPermissions = {},
  currencyMap?: Map<string, CurrencyInfo>,
  t?: TFunc,
): ColumnDef<DeliveryNoteListItem>[] {
  ...
  return [
    {
      key: "name",
      header: tr("deliveryNo"),
      sortKey: "name",
      render: (row) => (
        <Link
          href={`/delivery-notes/${encodeURIComponent(row.name)}`}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link href={`/delivery-notes/${encodeURIComponent(row.name)}`}>
            ...
            {row.docstatus === 1 && (
              <>
                <DropdownMenuItem asChild>
                  <Link href={`/sales-invoices/new?from_dn=${encodeURIComponent(row.name)}`}>
