...
function ModuleTab({
  ...
  return (
    ...
    >
      ...
      {isActive && (
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-teal-500" style={{ width: `${progress}%` }} />
      )}
    </motion.button>
  );
}
...
export function ModulesHero() {
  ...
  const reducedMotion = useSyncExternalStore(subscribeToReducedMotion, getReducedMotion, getServerSnapshot);
