...
import { MoreHorizontal, Pencil, Trash2, Send, X, Copy, RotateCcw } from "lucide-react";
...
export function getPurchaseInvoiceColumns(
  ...
  t?: TFunc,
): ColumnDef<PurchaseInvoiceListItem>[] {
  ...
  return [
    {
      ...
      render: (row) => (
        <Link
          href={`/purchase-invoices/${encodeURIComponent(row.name)}`}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link href={`/purchase-invoices/${encodeURIComponent(row.name)}`}>
            ...
            {row.docstatus === 1 && (
              <>
                {!row.is_return && (
                  <DropdownMenuItem asChild>
                    <Link
                      href={`/purchase-invoices/new?return_against=${encodeURIComponent(row.name)}`}
                    >
                ...
                <DropdownMenuItem asChild>
                  <Link href={`/purchase-invoices/new?amend_from=${encodeURIComponent(row.name)}`}>
