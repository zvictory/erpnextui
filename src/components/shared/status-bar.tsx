...
export function StatusBar({ type, message, linkHref, linkText }: StatusBarProps) {
  ...
  return (
    ...
    >
      {type === "loading" && <Loader2 className="size-4 shrink-0 animate-spin" />}
      {type === "success" && <CheckCircle className="size-4 shrink-0" />}
      {type === "error" && <XCircle className="size-4 shrink-0" />}

      <span className="flex-1">{message}</span>

      {type === "success" && linkHref && linkText && (
        <a href={linkHref} className="font-medium underline underline-offset-2 hover:no-underline">
          {linkText}
        </a>
      )}
    </div>
