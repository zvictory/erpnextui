  ...
  return [
    {
      ...
      render: (row) => (
        <Link
          href={`/purchase-orders/${encodeURIComponent(row.name)}`}
    ...
    },
    {
      ...
      header: tr("received"),
      className: "text-right",
      render: (row) => `${Math.round(row.per_received ?? 0)}%`,
    },
    {
      key: "status",
      header: tr("status"),
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link href={`/purchase-orders/${encodeURIComponent(row.name)}`}>
            ...
            {row.docstatus === 1 && (
              <>
                <DropdownMenuItem asChild>
                  <Link href={`/purchase-invoices/new?from_po=${encodeURIComponent(row.name)}`}>
                ...
                <DropdownMenuItem asChild>
                  <Link href={`/purchase-orders/new?amend_from=${encodeURIComponent(row.name)}`}>
