...
import { getSalesInvoiceColumns } from "@/components/sales-invoices/sales-invoice-columns";
import {
  useSalesInvoiceList,
  useSalesInvoiceCount,
...
export default function SalesInvoicesPage() {
  ...
  const { data: invoices = [], isLoading } = useSalesInvoiceList(
    company,
    listState.page,
  ...
  const { data: totalCount = 0 } = useSalesInvoiceCount(
    company,
    listState.debouncedSearch,
    extraFilters.length ? extraFilters : undefined,
  );
  ...
  const columns = getSalesInvoiceColumns(
    {
    ...
    {
      canDelete: canDelete("Sales Invoice"),
      canSubmit: canSubmit("Sales Invoice"),
      canCancel: canCancel("Sales Invoice"),
    },
    currencyMap,
    t,
  );
  ...
  const bulkActions: BulkAction<SalesInvoiceListItem>[] = [
    ...(canSubmit("Sales Invoice")
      ? [
          {
            label: tc("bulkSubmit"),
            onAction: (rows: SalesInvoiceListItem[]) =>
              setBulkConfirm({ type: "submit" as const, names: rows.map((r) => r.name) }),
          },
        ]
      : []),
    ...(canCancel("Sales Invoice")
      ? [
          {
            label: tc("bulkCancel"),
            variant: "destructive" as const,
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable<SalesInvoiceListItem>
        columns={columns}
        data={invoices}
        isLoading={isLoading}
        ...
        onRowClick={(row) => router.push(`/sales-invoices/${encodeURIComponent(row.name)}`)}
        filterBar={
          <ListFiltersBar
